...
### ✅ Full protocol-path scene (shrine clearing)
First scene generated end-to-end via the complete authoring protocol — not hand-authored Three.js. Demonstrates Path A working in practice.
- `examples/shrine_clearing.html` — 39 pieces: altar, 12 trees (4 clusters), 14 approach pavers, 6 lanterns, 4 rubble stones, 2 guardian statues
- Generated by: `DSL → solve_object_scene() → export_all_contexts() → author packets (per-piece, context-driven) → receive_all() → lidar verify → generate_scene_html(result, packets)`
- `examples/shrine_hero.png`, `shrine_walk.png`, `shrine_top.png` — auto-rendered by `visual_render.py`
- Status: **done.** All 39 packets validated, 0 verifier warnings, hero + walk screenshots confirm layout matches intent.
...
### ✅ visual_render.py — ES module scaffold support
`patch_html_for_camera_access` now handles both scaffold styles:
- Old UMD style (`<script src="/vendor/three.r128.min.js">`) — inlines Three.js for file:// loading
- New ES module style (`<script type="importmap">` with CDN URLs) — replaces importmap + module imports with local UMD r128 + OrbitControls stub; works offline, no server required
- Location: `scripts/verification/visual_render.py` → `patch_html_for_camera_access()`
- Status: **done.** Both scaffold variants tested and screenshotting correctly.
