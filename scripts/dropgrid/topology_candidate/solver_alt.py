...
def solve_compiled(spec, seed=42, debug=False, pre_pieces=None, pre_occ=None, reference_points=None):
    pieces = list(pre_pieces or [])
    trace = []
    occ = dict(pre_occ or {})
    warnings = list(spec.get('warnings', []))
    reference_points = dict(reference_points or {})

    anchor_type = spec['anchor']['type']
    anchor_label = spec['anchor']['label']
    next_id = max([p.id for p in pieces], default=0) + 1
    ax, az = (14, 12)
    existing_anchor = next((p for p in pieces if p.label == anchor_label), None)
    if existing_anchor is None:
        next_id, anchor = place_piece(pieces, occ, next_id, anchor_type, anchor_label, ax, az, 0)
    else:
        anchor = existing_anchor
        ax, az = anchor.gx, anchor.gz
    ...
    for obj in spec['objects']:
        ...
        if has_path:
            ...
            prev_turn = 0
            ref = reference_points.get(obj.get('target_ref') or obj.get('from'))
            if ref:
                x, z = round(ref[0] - dx), round(ref[1] - dz)
            else:
